...
(*
  ...
  As long as we're writing complete definitions and not just
...
(*
  This is of course so common that there is syntax for it
...
let xs = [1; 3; 23; 5; 77]
(*
  In a list, the elements are separated by semicolons
...
let dot (x1, y1) (x2, y2) =
  x1 * x2 + y1 * y2
(* It doesn't just work on tuples of course *)
let alist = [1; 3; 4]
let (x :: xs) = alist
...
let review b = match b with
  | {author="Peter F. Hamilton"} -> "pretty good"
  | _ -> "Not so good"
